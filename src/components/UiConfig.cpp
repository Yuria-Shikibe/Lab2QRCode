...
void Ui::applyFont(QApplication &app, const Ui &ui) {
    ...
    if (QFile::exists(qFontFile)) {
      ...
      if (fontId != -1) {
        ...
        if (!loadedFamilies.isEmpty()) {
          ...
          spdlog::info("Loaded custom font from file: {} -> {}", fontFile,
  ...
  for (const QString &requestedFont : fallbackFonts) {
    ...
    spdlog::debug("Trying: {} -> Got: {}", requestedFont.toStdString(),
                  actualFont.toStdString());

    QString normalized = requestedFont.simplified().toLower();
    QString actualNormalized = actualFont.simplified().toLower();

    if (actualNormalized.contains(normalized) ||
        normalized.contains(actualNormalized)) {
      spdlog::info("Successfully set font: {} (requested: {})",
                   actualFont.toStdString(), requestedFont.toStdString());
  ...
  QFontInfo fontInfo(font);
  spdlog::warn("No suitable font found, using system default: {}",
               fontInfo.family().toStdString());
  app.setFont(font);
  g_appFont = font; // Store the application font
}
